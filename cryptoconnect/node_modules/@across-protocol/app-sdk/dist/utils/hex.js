import { concat, isAddress, isHex, padHex } from "viem";
export const DOMAIN_CALLDATA_DELIMITER = "0x1dc0de";
export function tagIntegratorId(integratorId, txData) {
    assertValidIntegratorId(integratorId);
    return concat([txData, DOMAIN_CALLDATA_DELIMITER, integratorId]);
}
export function getIntegratorDataSuffix(integratorId) {
    assertValidIntegratorId(integratorId);
    return concat([DOMAIN_CALLDATA_DELIMITER, integratorId]);
}
export function isValidIntegratorId(integratorId) {
    return (isHex(integratorId) &&
        // "0x" + 2 bytes = 6 hex characters
        integratorId.length === 6);
}
export function assertValidIntegratorId(integratorId) {
...
export function bytes32ToAddress(hex) {
    if (!isHex(hex)) {
        throw new Error("Invalid hex input");
    }
    //  already address
...
export function hasMessage(message) {
