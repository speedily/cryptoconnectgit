import { AccountNotFoundError, KernelV3AccountAbi, getEncodedPluginsData, isPluginInitialized } from "@zerodev/sdk";
...
export async function prepareAndSignUserOperations(clients, args_) {
    ...
    if (account.kernelPluginManager.regularValidator) {
        const isPluginEnabledPerChains = await Promise.all(accounts.map(async (account, index) => (await account.kernelPluginManager.isEnabled(account.address, action.selector)) ||
            (await isPluginInitialized(clients[index], account.address, account.kernelPluginManager.address))));
